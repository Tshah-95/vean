...
function Row({ label, value, tone }: { label: string; value: string; tone?: "gold" }) {
  return (
    <div className="flex items-center justify-between py-0.5 text-xs">
      ...
      <span className={`font-mono ${tone === "gold" ? "text-primary" : "text-foreground"}`}>
        {value}
      </span>
    </div>
  );
...
export function RightPanel({
  ...
  return (
    <>
      ...
      {clip ? (
        <div className="mt-2">
          <div className="mb-2 truncate text-xs text-foreground">
            {clip.label ?? basename(clip.resource)}
          </div>
          <Row label="in" value={String(clip.in)} />
          <Row label="out" value={String(clip.out)} />
          <Row label="length" value={String(clip.out - clip.in + 1)} />
          {audioStreams != null ? (
            <Row label="audio" value={hasAudio ? `${audioStreams} ch` : "none"} />
          ) : null}
          {hasAudio ? (
            <button
              type="button"
